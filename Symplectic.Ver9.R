...
horizontal_move <- function(tableaux,setorder,components,coordinate){
  ...
  # Case : n
    ...
    #new_A[[1]] <- sort(new_A[[1]],decreasing = FALSE)
    #new_D[[1]]<- sort(new_D[[1]],decreasing = FALSE)
    #components$B[[coordinate[,2]+1]] <- sort(components$B[[coordinate[,2]+1]],decreasing = FALSE)
    #components$C[[coordinate[,2]+1]] <- sort(components$C[[coordinate[,2]+1]],decreasing = FALSE)
    ...
    if(ncol(new_coadmissible_left_col)!=0){
      ...
      for(q in 1:ncol(new_coadmissible_left_col)){
        ...
        if(tableaux[q,2*(coordinate[,2]+1)-1]!=0){
          tableaux[q,2*(coordinate[,2]+1)-1] <-  new_coadmissible_left_col[1,q]
          r<-r+1
        }
      }
    }
    ...
    if(ncol(new_coadmissible_right_col)!=0){
      ...
      for(q in 1:ncol(new_coadmissible_right_col)){
        ...
        if(tableaux[q,2*(coordinate[,2]+1)-1]!=0){
          tableaux[q,2*(coordinate[,2]+1)-1] <-  new_coadmissible_right_col[1,q]
          r<- r+1
        }
      }
    }
...
Splited_Form <- function(tableaux,setorder,coordinate){
  ...
  for(u in 1:ncol(tableaux)){
    ...
    #repeat{
      #if(length(tableaux[,u])>=w&&tableaux[w,u]==0){
...
absolute_king <- matrix(,nrow=4,ncol=4)
...
#haha<-sjdt_Algorithm(wow[[10732]],4,4)
count <- 1

for(i in 1:length(D_Tableaux)){
print(i)

newShape[[count]]<-sjdt_Algorithm(D_Tableaux[[i]],4,4) 
count <- count+1
print(newShape[[count-1]])
#if(identical(newShape[[count-1]],newShape)==TRUE){
  #print(newShape[[count-1]])
  #print(count)
  #while(TRUE){
